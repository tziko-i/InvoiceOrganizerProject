...
public class InvoicesController(AppDbContext db) : ControllerBase
{
        ...
        )
    {
        ...
        // Query for categorized items
        var qItems = db.InvoiceItems.AsNoTracking().AsQueryable();

        if (!string.IsNullOrEmpty(userId))
            qItems = qItems.Where(ii => ii.Invoice != null && ii.Invoice.UserId == userId);

        if (supplierId.HasValue)
            qItems = qItems.Where(ii => ii.Invoice.SupplierId == supplierId.Value);

        if (start.HasValue)
            qItems = qItems.Where(ii => ii.Invoice.InvoiceDate >= start.Value);
        if (end.HasValue)
            qItems = qItems.Where(ii => ii.Invoice.InvoiceDate < end.Value);

        var data = await qItems
        ...
        // Query for uncategorized invoices ("כללי")
        var qInvoices = db.Invoices.AsNoTracking().AsQueryable();
        
        if (!string.IsNullOrEmpty(userId))
            qInvoices = qInvoices.Where(i => i.UserId == userId);

        if (supplierId.HasValue)
            qInvoices = qInvoices.Where(i => i.SupplierId == supplierId.Value);

        if (start.HasValue)
            qInvoices = qInvoices.Where(i => i.InvoiceDate >= start.Value);
        if (end.HasValue)
            qInvoices = qInvoices.Where(i => i.InvoiceDate < end.Value);

        var uncategorizedInvoices = await qInvoices
            .Where(i => !i.Items.Any())
            .ToListAsync();

        if (uncategorizedInvoices.Any())
        {
            var uncategorizedTotal = uncategorizedInvoices.Sum(i => i.Total);
            if (uncategorizedTotal > 0)
            {
                data.Add(new CategorySummaryDto
                {
                    CategoryId = -1,
                    CategoryName = "כללי",
                    Count = uncategorizedInvoices.Count,
